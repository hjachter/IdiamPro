...
const OV_USER_AGENT = 'IdeaM/1.0 (slideshow slide illustrations; https://secondbrainware.com)';
...
function writeMediaCredits(outputPath, credits) {
  ...
  try {
    const lines = [
      ...
      `Generated by IdeaM on ${new Date().toISOString()}`,
      '',
...
// (IdeaM wordmark + slide number), and AUTO-FIT: an inline script shrinks the
// content until it fits the safe area, so dense real-outline text never overflows.
// Brand accent is IdeaM's iOS blue (bright variant for dark-bg readability).
function slideHtml(slide, index, total, style, mindmapSvg, photoUrl, videoClip, screenshotUrl) {
  ...
  const brandLabel = typeof s.brandLabel === 'string' ? s.brandLabel.trim() : 'IdeaM';
  const logoDataUrl = typeof s.logoDataUrl === 'string' ? s.logoDataUrl.trim() : '';
  // Free-tier taste: non-Pro renders carry a subtle "Made with IdeaM" mark.
  ...
  const watermarkHtml = watermark
    ? `<div class="wm"><span class="wm-dot"></span>Made with IdeaM</div>`
    : '';
  const footerHtml = `
        <div class="footer">
          ${brandHtml}
...
async function generateSlideshowVideo(opts) {
  ...
  // Default output location: ~/Documents/IdeaM Videos/
  let outputPath = opts.outputPath;
  if (!outputPath) {
    const outDir = path.join(os.homedir(), 'Documents', 'IdeaM Videos');
