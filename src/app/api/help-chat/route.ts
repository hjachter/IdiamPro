...
- Generate Visual (content toolbar Image button): Insert a visual into a node. AI illustration (Google Imagen) is a Pro feature — each image counts as ONE generation against the monthly cap; free or over-cap users see the upgrade prompt instead of a paid call, so a paid image is never generated by accident (bring your own API key in Settings to bypass the cap). Mind Map and Flowchart are ALWAYS FREE — drawn locally from the current node's outline structure with no AI, and never counted.
