...
- Generate Video (desktop app, Pro): Open the Export menu (BookUp icon) > "Generate Video" to turn a chapter (a selected node + its sub-points) into a narrated slideshow MP4. Customizable Style section: upload your own logo, pick an accent color, choose dark/light theme, and pick a narrator voice. A "Slide visuals" control (Off, Mind maps, Photos, or Auto — Auto is the default) can add visuals to each slide: mind maps drawn automatically from that part of your outline, free public-domain photos fetched to match each slide, or Auto (mind maps for sections plus photos for detail slides). Photos come only from license-clean, free public-domain sources (Openverse, CC0/Public Domain), cost nothing, and a small image-credits file is saved next to the video; if a photo can't be fetched the slide simply falls back to text or a mind map. A "Detail" control (Overview, Standard, Deep, or Full outline) sets how deep into the outline the video goes — Standard covers sections and subsections, while Full outline turns the ENTIRE outline, at any depth, into a video. Slides render automatically, an AI voiceover (OpenAI TTS) narrates them, and everything is stitched into a YouTube-ready MP4 saved to Documents · IdiamPro Videos. Rendering runs entirely on the Mac desktop app (nothing uploaded); the web and iOS builds show a "use the desktop app" notice. Without an OpenAI key the video still renders, just with silent slides. Everyone gets 10 free videos to try it (those free videos carry a small "Made with IdiamPro" watermark); Pro unlocks unlimited videos with no watermark. (Full web/iOS rendering is planned for a future release.)
...
- Generate Video (desktop, Pro): Export menu > "Generate Video" turns a chapter into a narrated slideshow MP4 with your own branding (logo, accent color, dark/light theme, narrator voice), a "Slide visuals" control (Off / Mind maps / Photos / Auto, Auto by default) that can draw a mind map of each section, add a free public-domain photo (CC0/Public Domain, no cost, with a credits file saved beside the video), or Auto-mix mind maps for sections and photos for details, and a "Detail" control (Overview / Standard / Deep / Full outline) for how deep into the outline the video goes — Full outline covers every level. Slides render on-device, an AI voiceover (OpenAI TTS) narrates them, and ffmpeg stitches them into a video saved to Documents · IdiamPro Videos. Desktop-only rendering; everyone gets 10 free videos to try (free videos carry a small "Made with IdiamPro" watermark), Pro unlocks unlimited unmarked videos; silent slides if no OpenAI key.
