...
- Wizards (one-click automated workflows): open the Smart Tools menu (sparkles icon) and pick "Wizards" to run a whole recipe with a single Run press. Each Wizard asks a few friendly questions that shape the output (or just hit Run for smart defaults). "Automatic Book" is live today — a short guided dialogue (topic, depth, tone, audience) produces a full outline with AI-written sections as a brand-new outline. Podcast from Anything, Research Digest, and Instant Study Guide are previews (coming soon) in this build. Wizards run on the app's built-in AI and are free-tier friendly; premium generation packages come later.
...
- Export/Share: Select a node and use Export menu (BookUp icon in toolbar) > "Share Suboutline as…" — or right-click any node > "Share Suboutline As…" for the same dialog. Formats include PDF, Markdown, Plain Text, HTML (collapsible webpage), Interactive Outline (read-only IdiamPro-style viewer with sidebar navigation, search, dark/light mode), Website (8 professional templates: Marketing, Informational, Documentation, Portfolio, Event, Educational, Blog, Personal), Podcast (AI-generated audio), OPML, Obsidian (wiki-links), CSV, JSON Tree.
...
- Generate Video (desktop, Pro): Export menu > "Generate Video" turns a chapter into a narrated slideshow MP4 with your own branding (logo, accent color, dark/light theme, narrator voice), a "Slide visuals" control with three combinable checkboxes (Mind maps / Photos / Video clips; default Mind maps + Photos on, Video clips off) and a "Detail" control (Overview / Standard / Deep / Full outline) for how deep into the outline the video goes — Full outline covers every level. Slides render on-device and ffmpeg stitches them into a video saved to Documents · IdiamPro Videos; while rendering, a live progress bar and an estimated time remaining are shown. Videos ALWAYS narrate — OpenAI TTS if a key is set, otherwise the Mac's free built-in system voice, so it's never silent. Desktop-only rendering; everyone gets 10 free videos to try (free videos carry a small "Made with IdiamPro" watermark), Pro unlocks unlimited unmarked videos.
...
export async function POST(request: NextRequest) {
  // Approval + rate limit (per-IP AND per-user, 30/min chat tier). Only an
  // approved (or dev/stub) caller reaches the AI; unapproved → 403.
