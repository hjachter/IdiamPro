...
- Ctrl+F (Cmd+F on Mac): Search the outline as a view-shaper. When the user submits a search, the tree reshapes to show ONLY the nodes matching the term plus the chain of ancestors leading to each one. Every other node is removed from the visible row list entirely (not just collapsed under a parent — actually hidden). A status chip appears above the tree: 'Filtered: "[your term]" · [N] hidden · Clear'. WHAT CLEARS THE FILTER (everything visible again): submitting a new search (recalculates from scratch), Expand All (Cmd+E / Ctrl+E), Collapse All (Cmd+Shift+E / Ctrl+Shift+E), or clicking Clear on the chip. WHAT PRESERVES THE FILTER (hidden nodes stay hidden): pressing Escape to close the search bar, clicking a chevron on a visible node to collapse or expand just that node, editing a node's name or content, drag-reordering visible nodes, copy/cut/paste, and moving the selection. New nodes created while filtered are visible (not in the hidden set). The filter is persisted with the outline and undoable. The funnel icon toggles "Search only open nodes" — when on, the next search walks only currently visible nodes, letting you chain queries as a logical AND ("alpha" then "beta" -> nodes containing both). Filter toggles let you search node names only, content only, or both. Scope toggle picks current outline or all outlines.
...
- Transform outline with AI (whole-subtree structural transformation, 2026-06-06): Restructure a subtree (or the entire outline) by describing what should change in plain English. This is the structural counterpart to Reformat with AI: Reformat rewrites ONE node's content body; Transform changes the SHAPE of the outline — adding, removing, renaming, merging, splitting, or moving nodes. Open from the Smart Tools menu > "Transform outline with AI…" or from the Command Palette > "Transform outline with AI". Scope rule: if a node is selected, the transform operates on that node and everything beneath it; if nothing is selected, it operates on the whole current outline (root down). The dialog: (1) type how to transform the subtree — example chips include "Reorganize alphabetically by name", "Merge chapters with fewer than three children into a Misc chapter", "Promote leaf nodes about [topic] to top-level chapters", "Convert each paragraph node into a heading with its body as a child", "Deduplicate near-duplicate nodes", "Extract everything tagged [tag] into its own new chapter"; (2) click Preview transform; (3) the AI returns a transformed version and the dialog shows a before/after structural tree view with color coding — green = added, red strike-through = removed, amber = renamed, blue = moved — plus a one-line summary like "I'll add 12, remove 4, rename 8, move 23 nodes."; (4) click Apply, Modify instruction, or Cancel. Large-subtree safeguard: if the subtree has more than 2,000 nodes, the dialog surfaces an inline warning that very large transforms can be unreliable and suggests narrowing scope — the user can still proceed. Contract enforced server-side: existing node IDs are preserved for anything kept; new nodes get fresh UUIDs (the model uses NEW_NODE_[seq] placeholders that the server action replaces); the subtree's root parentId anchor is restored from the original outline so the subtree stays in its place; circular parent/child relationships are rejected; the root is never deleted. Optional "Use local AI (Ollama)" tickbox keeps the transform on-device. One transform = ONE generation against the monthly cap, regardless of subtree size. NOT a Pro-only feature.
