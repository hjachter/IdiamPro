...
- LIVE BOOKS (Refresh from the web): Manually refresh a node and all its descendants against the latest information so documents don't go stale. Open it from the AI menu > "LIVE BOOKS: Refresh from the web", the Command Palette, or Cmd/Ctrl+Shift+R (select the root node to refresh the entire outline). Choose Merge & augment (default — keep accurate content, fix outdated parts, fold in new info) or Overwrite/regenerate. By default every change is shown in a side-by-side preview with per-node accept/reject and nothing is applied until you approve; "Apply automatically without previewing" is an explicit opt-in that is off by default. Nodes you edited by hand are auto-skipped to protect your writing, with a per-node "Include anyway" override. Every refreshed node stores its sources and the model used — a compact "Sources" chip on the node shows the citations and attribution (e.g. "Refreshed with Gemini 3.5 Flash"), and this travels with the outline through save/export. Cloud AI uses live web search for real citations; local AI uses the model's built-in knowledge (no internet) and says so honestly rather than inventing sources. LIVE BOOKS is personal/local — it only changes your own copy after approval. It is built on a reusable transform engine that the Translate feature also uses.
- Translate this section (language translation, #52): Translate any selected node and its descendants into another language with the same preview-and-approve safety as LIVE BOOKS. Open from the AI menu > "Translate this section" or from the Command Palette. The language dropdown is ordered for IdiamPro's globally-distributed-team users — Chinese (Simplified), Portuguese (Brazilian), Spanish, French, Japanese, Korean, German, Italian, Hindi appear first; a wider catalog follows. There is NO default language — the user must pick one before the Translate button enables. Tick "Use local AI (Ollama)" to keep the translation entirely on-device. Click Translate & preview: each node is shown side-by-side with the original. Reject any you don't want, then click Apply to commit. Formatting (headings, lists, bold, links) is preserved; proper nouns and code stay as-is. Nodes you edited by hand are auto-skipped with the same Include-anyway override. One Translate counts as ONE generation against the usage cap, regardless of how many nodes are in the subtree.
