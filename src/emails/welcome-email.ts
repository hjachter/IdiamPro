...
export interface WelcomeEmailProps {
  ...
  /** Optional CTA URL — defaults to the IdeaM app home. */
  appUrl?: string;
...
export function renderWelcomeEmail(props: WelcomeEmailProps): RenderedEmail {
  ...
  const bodyHtml = `
<h1 style="margin:0 0 16px;font-size:24px;line-height:1.3;font-weight:700;">Welcome to IdeaM</h1>
<p style="margin:0 0 16px;">${escapeHtml(hello)} &mdash; thanks for joining. IdeaM turns scattered notes, videos, PDFs, and half-formed ideas into structured outlines you can actually use.</p>
...
<ol style="margin:0 0 24px;padding-left:20px;">
  ...
  <li style="margin-bottom:8px;"><strong>Try the AI menu.</strong> Click the sparkles icon in the toolbar. Generate a branch from a topic, refresh a section against the latest web sources, or ask your own outlines a question.</li>
...
`;
  ...
  const bodyText = `${hello} — thanks for joining IdeaM.

IdeaM turns scattered notes, videos, PDFs, and half-formed ideas into structured outlines you can actually use.
...
2. Try the AI menu. Click the sparkles icon in the toolbar. Generate a branch from a topic, refresh a section against the latest web sources, or ask your own outlines a question.
