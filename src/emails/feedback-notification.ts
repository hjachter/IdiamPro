...
export function renderFeedbackNotification(
  ...
  if (f.bestThing) {
    sections.push(`<h2 style="font-size:18px;margin:24px 0 8px;">Best thing about IdeaM</h2><blockquote style="margin:0 0 16px;padding:8px 12px;border-left:3px solid #007AFF;color:#374151;">${escapeHtml(f.bestThing)}</blockquote>`);
  }
  if (f.biggestWish) {
...
<ul style="margin:0 0 16px;padding-left:20px;">
  ...
  if (f.toolsBeforeIdiampro) txt.push('', `Tools before IdeaM: ${f.toolsBeforeIdiampro}`);
