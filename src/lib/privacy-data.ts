 ...
 *
 * Today, IdeaM/SecondBrainWare stores all user data locally on the
...
/**
 * Known IdeaM localStorage keys. Anything not in this list is
 ...
 * belong to IdeaM (e.g. per-provider API keys).
 */
...
async function buildExportZip(): Promise<{ blob: Blob; filename: string }> {
    ...
    JSON.stringify(
      {
        ...
        other_keys: unknown,
        note:
          'app_keys are IdeaM/SecondBrainWare keys. other_keys are unrelated keys present on the same origin and are included for completeness only.',
      },
      null,
      2
    )
  ...
  const manifest: ExportManifest = {
    ...
    notes:
      'This archive contains all data IdeaM stores locally on your device. There is no server-side account today, so this is a complete export. Outlines are in outlines/*.idm (JSON). Settings and AI consent are in settings/.',
  };
  ...
  zip.file(
    'README.txt',
    [
      'IdeaM / SecondBrainWare — Personal Data Export',
      '',
      ...
      '',
      'To restore: open IdeaM and use the admin menu > "Restore All',
...
async function saveExportZip(blob: Blob, filename: string): Promise<'saved' | 'cancelled'> {
  ...
  if (isCapacitorNative()) {
    ...
    await Share.share({
      title: 'IdeaM Data Export',
      text: 'Your IdeaM data archive',
      url: written.uri,
      dialogTitle: 'Save your IdeaM export',
    });
...
/**
 * Remove every IdeaM-owned localStorage key. Unknown keys are left
