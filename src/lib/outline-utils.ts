...
export function parseMarkdownToNodes(markdown: string, topic: string): { rootNodeId: string, nodes: NodeMap } {
  ...
  // Detect format: heading-based (# / ##) or bullet-based (- )
  const allLines = markdown.split('\n');
  const hasHeadings = allLines.some(line => /^#{1,6}\s/.test(line.trim()));

  if (hasHeadings) {
    // Parse heading-based markdown (# Section, ## Subsection, etc.)
    // Collect content lines between headings
    let currentHeadingLevel = 0;
    let pendingContent: string[] = [];

    const flushContent = () => {
      if (pendingContent.length > 0 && parentStack.length > 1) {
        const lastNodeId = parentStack[parentStack.length - 1];
    ...
    for (const line of allLines) {
      ...
      if (headingMatch) {
        ...
        // level 1 = child of root (stack depth 1), level 2 = child of level 1, etc.
        ...
        // Non-heading line — collect as content for the current heading
        const trimmed = line.trim();
        if (trimmed || pendingContent.length > 0) {
    ...
    // Parse bullet-based markdown (- item with indentation)
