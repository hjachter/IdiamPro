...
export function getMicPermissionHelp(ctx?: PlatformContext): string {
  ...
  if (runtime === 'electron') {
    if (os === 'macos') {
      return 'Open System Settings → Privacy & Security → Microphone, then enable IdiamPro (or Electron) and restart the app.';
    }
    if (os === 'windows') {
      return 'Open Settings → Privacy & security → Microphone, then enable IdiamPro and restart the app.';
    }
    if (os === 'linux') {
      return 'Check your audio settings (PulseAudio/PipeWire) and make sure IdiamPro has microphone access, then restart the app.';
    }
    return 'Check your system’s microphone permissions for IdiamPro.';
  }

  if (runtime === 'capacitor-ios') {
    return 'Open the iOS Settings app → IdiamPro → Microphone, and turn it on.';
  }
  if (runtime === 'capacitor-android') {
    return 'Open Android Settings → Apps → IdiamPro → Permissions → Microphone, and turn it on.';
  }
