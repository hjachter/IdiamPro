...
// App context for AI to understand what IDMPro does
const APP_CONTEXT = `You are a helpful assistant for IDMPro, a professional outlining application with AI-powered features.
...
- Cmd+Shift+F: Focus Mode (isolate suboutline, Esc to exit). Also available as a Focus button in the outline toolbar — highlights when active, disabled when no node is selected.
...
- Generate Video (desktop, Pro): Export menu > "Generate Video" turns a chapter into a narrated slideshow MP4 with your own branding (logo, accent color, dark/light theme, narrator voice), a "Slide visuals" control with three combinable checkboxes (Mind maps / Photos / Video clips; default Mind maps + Photos on, Video clips off) and a "Detail" control (Overview / Standard / Deep / Full outline) for how deep into the outline the video goes — Full outline covers every level. Slides render on-device and ffmpeg stitches them into a video saved to Documents · IDMPro Videos; while rendering, a live progress bar and an estimated time remaining are shown. Videos ALWAYS narrate — OpenAI TTS if a key is set, otherwise the Mac's free built-in system voice, so it's never silent. Desktop-only rendering; everyone gets 10 free videos to try (free videos carry a small "Made with IDMPro" watermark), Pro unlocks unlimited unmarked videos.
...
- Onboarding welcome (2026-07-10): Brand-new users see a single, skippable "What you can make here" panel the first time they open the app — a value showcase of the app's outputs (Video, Podcast, Website, Docs/Export) and inputs (Import from YouTube/PDFs/web, AI + Second Brain). It shows ONCE, has a "Get started" and a "Skip" button, never reappears after dismissal (localStorage flag onboarding:welcomeShowcaseSeen), and is fully suppressed in Professional mode. Lives at [src/components/welcome-showcase.tsx].
...
- IDMPro includes an MCP (Model Context Protocol) server for programmatic access to outlines
...
ONBOARDING EMAILS:
- When a user signs up (Clerk's user.created webhook fires) IDMPro sends a four-email onboarding series, all branded from "IDMPro [welcome@2ndbrainware.com]" via Resend.
...
- Sign-up flow: click "Sign up to try IDMPro free" on the homepage hero. No credit card required. New users land in /app on the Free trial tier (25 generations one-time).
...
- Sharing a quote we can put on the IDMPro website earns the user a Founding User badge inside the app (driven by uploading a 30-second video testimonial).
...
- IDMPro is in invite-only beta. Every prospective user fills out a short application at /signup and Howard personally approves each one before they can use the app.
- The application form collects name, email, and an optional "What brings you to IDMPro?" textarea. Submitting POSTs to /api/applicants/apply, which persists the applicant record (file-based JSON store at .idiampro/applicants.json) and emails Howard at howard@2ndbrainware.com with the details and a deep link to /admin/applicants?focus=[id].
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hi! I\'m here to help you with IDMPro. Ask me anything about features, keyboard shortcuts, or how to use the app!',
      timestamp: Date.now(),
    },
  ]);
      ...
      <DialogContent className="max-w-2xl h-[600px] flex flex-col p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>IDMPro Help</DialogTitle>
              <DialogDescription>
