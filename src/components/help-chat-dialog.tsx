...
KEY FEATURES:
- HOW CONTROLS ARE ORGANIZED (the one rule, same in both panes): a control that acts on the SELECTED item / selected text / cursor position lives in a right-click (context) menu; a control that acts on the WHOLE outline or is global lives on the TOOLBAR. So: Add / Rename / Delete / Cut-Copy-Paste / Expand this item / Compress this item / Zoom In (focus this branch) / Refresh from Web / Properties / Export this item are on the node right-click menu; New Outline, Search, AI, Import, Export (export whole outline), Second Brain, Expand All / Compress All, and Help are on the toolbar. Open/close vocabulary: "Expand" opens one item and leaves others as-is; "Compress" closes one item; "Zoom In / Focus" opens one item and hides everything outside its branch (with a breadcrumb to exit); "Expand All / Compress All" act on the whole outline; "Search / Find" is a smart Expand All that opens only matching items.
- Settings is organized into categories: General, Data, Professional Customization, AI, Privacy & Data, Account, Backups, and About.
- Settings > Data (right after General) shows WHERE your outlines are stored: the current folder path is always visible, with a "Choose Folder…" button to move it. Tip: pick a folder in iCloud Drive and outlines follow you between Macs (best used on one Mac at a time — simultaneous edits can conflict). On iOS, outlines are stored in the app automatically.
- Settings has a search box pinned at the top: type any word (e.g. "folder", "backup", "key") and matching settings from every section appear in one flat list; click a result to jump to its section. Esc or the clear button restores normal browsing.
...
- Project Management reports (PURE LOGIC, NO AI, no cost): when the Project Management capability is ON (Settings > Professional Customization > Capabilities), the Wizards gallery shows an extra "Project Management" group marked "No AI". Each report reads the task statuses and dependencies you've already set and drops a read-only summary note into your current outline — it calls no AI, costs nothing, and never modifies a task. Five reports: "What's Blocking Me" (tasks stuck behind a prerequisite that isn't Done, naming each blocker), "In Progress" (status = In progress), "Waiting / Blocked" (Not started, or blocked by an unfinished prerequisite), "Completed" (status = Done), and "Status Overview" (counts by status + percent complete). The group is hidden while Project Management is off.
- AI-powered outline generation from topics
- Automatic quality check (always on, 2026-07-23): Every AI draft of a FACTUAL output — Export Email, social posts (X/Instagram/LinkedIn/Facebook/Threads/Bluesky/YouTube), Summarize, and Import Email — is automatically checked against its source (your outline branch, or the email it came from) for claims the source doesn't support (possible AI hallucinations). Suspect claims are flagged in the preview under a "Please review these" note, with an optional corrected version you can accept or edit — it never rewrites your draft silently, and you always review before you send or publish. The check runs on the on-device AI (Ollama/Gemma), so it's free and never uses up your monthly AI generations; where on-device AI isn't available (web / iPhone / iPad) the automatic check is skipped and you still see the reminder to review. AI can still make mistakes — this is a helpful safety net, not a guarantee.
...
- Outline list sort (2026-07-23): A small "Recent / Name" toggle sits in the Outlines header (both the desktop sidebar and the mobile sidebar sheet). "Recent" orders outlines by most-recently-edited first — the DEFAULT — so you land back on whatever you were just working on without hunting; "Name" is the classic A–Z order. The choice is remembered across sessions and applies to both the list and the sidebar identically. Outlines with no last-modified timestamp sort to the bottom in Recent mode.
...
- Export/Share: Select a node and use Export menu (up-arrow icon in toolbar) > "Share Suboutline as…" — or right-click any node > "Share Suboutline As…" for the same dialog. Formats include PDF, Markdown, Plain Text, HTML (collapsible webpage), Interactive Outline (read-only IdeaM-style viewer with sidebar navigation, search, dark/light mode), Website (8 professional templates: Marketing, Informational, Documentation, Portfolio, Event, Educational, Blog, Personal), Podcast (AI-generated audio), OPML, Obsidian (wiki-links), CSV, JSON Tree.
...
- Generate Video (desktop app, Pro): Open the Export menu (up-arrow icon) > "Generate Video" to turn a chapter (a selected node + its sub-points) into a narrated slideshow MP4. Customizable Style section: upload your own logo, pick an accent color, choose dark/light theme, and pick a narrator voice. A "Slide visuals" control offers three combinable, independent checkboxes — Mind maps, Photos, and Video clips (default: Mind maps + Photos on, Video clips off) — that mix per slide: sections favor a mind map, detail slides favor a video clip if that box is ticked otherwise a photo, the cover gets a photo/clip if enabled, and unchecking all three gives clean text-only slides. Video clips is off by default because clip relevance can't be guaranteed; when it's on and no strong match is found the slide quietly uses a photo instead. Mind maps are drawn automatically from that part of your outline; photos and clips come from free public-domain sources. Photos and clips come only from license-clean, free public-domain sources (photos from Openverse CC0/Public Domain; clips from Wikimedia Commons public-domain/CC0 with no key needed, or from Pexels/Pixabay if you've set a free key) — everything is free with no per-item charge, and a small media-credits file is saved next to the video. Every fetch fails safe: if a clip can't be had it falls back to a photo, then to text, so a missing visual never breaks the video or costs anything. A "Detail" control (Overview, Standard, Deep, or Full outline) sets how deep into the outline the video goes — Standard covers sections and subsections, while Full outline turns the ENTIRE outline, at any depth, into a video. Slides render automatically, an AI voiceover (OpenAI TTS) narrates them, and everything is stitched into a YouTube-ready MP4 saved to Documents · IdeaM Videos. While it renders, the dialog shows a live progress bar (e.g. "Rendering slide 3 of 12", "Stitching video") and an approximate time remaining that updates as it goes. Rendering runs entirely on the Mac desktop app (nothing uploaded); the web and iOS builds show a "use the desktop app" notice. The video ALWAYS narrates and is never silent: with an OpenAI key it uses natural AI text-to-speech, and without one it falls back to the Mac's built-in system voice (free, offline). Everyone gets 10 free videos to try it (those free videos carry a small "Made with IdeaM" watermark); Pro unlocks unlimited videos with no watermark. (Full web/iOS rendering is planned for a future release.)
- Make a Slide Deck (Export menu > "Slide Deck"): Turn an outline into a branded slide deck — a real PowerPoint .pptx file that opens natively in BOTH Microsoft PowerPoint AND Apple Keynote (and Google Slides), with no conversion and no lock-in. Each chapter/section becomes a slide with a concise title and tidy bullets, styled with your brand look (your logo, an accent color, and a light or dark theme on every slide). Numeric data is AUTO-CHARTED: when a section contains a small table or a list of figures, IdeaM turns those numbers into a clean chart on the slide instead of a wall of digits. A depth control sets how far into the outline the deck goes; the finished .pptx saves to your device, ready to present or fine-tune in whatever app you already use. A live example deck is hosted on the marketing site at /ideam-seminar.pptx ("See our presentation" link on the home page).
- Outline management: The sidebar (panel-toggle button at top-left of toolbar) is the sole path for switching, creating, renaming, deleting, and searching outlines. The toolbar shows the current outline name as a read-only title plus two small icon buttons: Import (down-arrow) and Export (up-arrow). Import menu contains: Research & Import, Import Outline, Restore All Outlines (plus "Import Email" when inbound email import is enabled in Settings > Professional Customization, and "Suggestions (N)" when an outside AI assistant has proposed changes waiting for review — see the Suggestions bullet below). Export menu contains: Share Suboutline as…, Export Current Outline, Backup All Outlines. (The previous Wrench / Refresh User Guide menu was retired 2026-06-07 — the User Guide is now read-only and reloads from the bundled app version on every launch, so a manual refresh is no longer needed.)
- Suggestions from AI assistants (desktop only): outside AI assistants connected through the IdeaM connector (like Claude Desktop or Claude Code) can READ outlines but can never change them — their proposed changes (add / rewrite / move / delete an item, or a whole new outline draft) wait as suggestions. When suggestions exist for the open outline, a small green dot appears on the Import button and the Import menu shows "Suggestions (N)". Each suggestion is described in plain English (which assistant, when, what it wants to do) and reviews through the SAME approve-before-apply engine as the app's own AI: additions appear as provisional green Pending items, deletions mark the item and everything inside it in amber strike-through, rewrites show a Before/After comparison, moves highlight the item in blue ("Will move"), and a new-outline draft gets a preview with "Add to my outlines" / Discard. Every approval applies through the normal editing path so Undo (Cmd+Z) works afterward. If the outline changed after a suggestion was made, it is marked "The outline changed since this was suggested" and can only be dismissed. Nothing changes unless the user approves it here.
...
- Import: Multi-format import via the toolbar Import menu (down-arrow icon) > "Import Outline". Supports Markdown (.md - heading hierarchy), Plain Text (.txt - indentation), OPML (.opml - standard outline XML), JSON/IDM (native format). Drag-and-drop or browse to select. Auto-detects format.
...
- Board View (2026-09-11): right-click any node > "Board View" to see that node's subtree as a kanban-style board — its children become the columns and their children become the cards. Dragging a card to another column ACTUALLY MOVES the item to that section of the outline (the board is a live projection of the outline, not a separate copy); dropping on the top/bottom half of another card sets its position among siblings, and Cmd+Z undoes any move. Cards show name, color stripe, tags, and a count of items inside. Free, local, no AI. Current version is move-only — rename/add/edit still happen in the outline.
...
- Filter by tag (2026-07-26): A tag icon in the outline toolbar (right next to Search) opens a tag-filter panel. It lists every tag used in the CURRENT outline as a colored chip. Tapping one or more chips narrows the outline to just the items carrying those tags (OR logic — the more chips you pick, the more you see), keeping each match's ancestors visible for context and its descendants so the branch shows in full; everything else is hidden. The eraser button clears the filter and restores the whole outline; the tag toolbar button highlights while a filter is active. Works by tap or click on every platform (no hover needed). This is a live view filter, not a search — it complements Ctrl+F.
- Show Me / custom views (2026-09-09): AI menu (sparkles button) → "Show Me…" opens a slim panel under the toolbar for natural-language filtering with VISIBLE, EDITABLE criteria. Type a request like 'open tasks about design from August' and press Enter — one quick AI call translates it into structured condition rows (field / comparison / value) the user can inspect, edit, remove, or extend. Conditions can also be built entirely BY HAND with "Add condition" (no AI, free-tier safe); fields cover item name, content, tag, item type, completed, priority, color, created/updated/due dates, and 'under branch named'. With several rows, choose match ALL or ANY. Results use the classic compress-don't-hide reshaping: matches are highlighted and unfolded with their ancestors; every other branch is folded but remains a visible row the user can chevron-open — nothing is hidden or removed (Expand All restores the full view). The view is LIVE: highlights and the match count re-check as the outline changes, so a newly added matching item lights up immediately. "Save view" stores the conditions under a name; saved views appear as chips in the panel, are PER-OUTLINE, and reopening one re-evaluates against the outline's CURRENT items — a durable query, never a snapshot. Delete a saved view via the trash icon on its chip. The AI only translates words into conditions; filtering itself always runs locally on-device.
- Status tags / project management (2026-07-27): Built on the tag system, IdeaM has a reserved set of four STATUS tags with fixed colors and meaning — Not started (red), In progress (amber), Done (green), Blocked (grey). Assign one from the node's context menu: right-click a node (long-press on iPad/iPhone) → the "Project" submenu → "Set Status" → pick a status, or "Clear Status" to remove it. Status is MUTUALLY EXCLUSIVE: a node has at most one status at a time, and setting a new one automatically replaces the old (free-form tags are left untouched). It operates on the CURRENT SELECTION — a single node, or a multiselection (select several nodes first, then set the status to bulk-apply it to all of them). Status shows as a colored badge like other tags, and because it IS a tag, you can narrow the outline to a status with the Filter-by-tag control (e.g. show only "In progress" items). Cross-platform: right-click on Mac/PC = long-press on iPad/iPhone.
- Prerequisites / task dependencies (2026-07-27): The Project submenu also has "Set Prerequisite…" — this lets you say a task DEPENDS ON one or more other tasks in the same outline (they must be Done first). Right-click a node → Project → Set Prerequisite… opens a searchable picker of the outline's other items; tap items to add or remove them as prerequisites (a node can't depend on itself or its own descendants). The dependencies are stored as structured, machine-readable data on the node (not text in the content), so they travel with the outline and can be queried. In the content pane, an item's prerequisites appear as clickable "Depends on" chips that jump to that task, colored green when Done and amber when not; you can add ("+ Add") or remove (the × on a chip) them there too. BLOCKED SURFACING: if any prerequisite is NOT yet Done, the item shows a "⚠ Blocked by: <name>" indicator in the outline and content pane. This is a COMPUTED indicator only — it never changes or overrides the status you set by hand; it just warns you the task isn't ready to start. It clears automatically once every prerequisite is marked Done. Cross-platform: right-click on Mac/PC = long-press on iPad/iPhone.
- Task node type + Priority + Due date (2026-07-27): With Project Management ON, the green "+ ▾" new-item dropdown (click the chevron beside the green + in the outline toolbar) gains a "New Task" option. It creates a typed TASK item (shown with a checkbox) that comes PRE-LOADED with a project-management template at the top of its content pane — a light inline "Task details" form showing four fields: Status (the same four reserved status tags — Not started / In progress / Done / Blocked), Depends on (the same prerequisites, shown as chips in the header just above), Priority (a NEW metatag: Low / Medium / High), and Due date (a NEW date picker). Priority and Due date are stored as structured, machine-readable metadata (metadata.priority and metadata.dueDate) — not text in the prose — so AI can read and compare them. Below the template, the normal free content editor works as usual. Both the "New Task" menu option AND the template are gated behind the Project Management capability: with PM OFF the "New Task" option disappears from the menu and the template is hidden, but any Priority / Due date / Status / Prerequisites already set are PRESERVED (never deleted) and reappear the moment PM is turned back on. Cross-platform.
...
- AI costs & approvals (cost classes): Every AI feature has a cost class — light (tag suggestions, AI commands, Help chat answers), medium (single-document work: expand, summarize, reformat, transform, translate, outline-from-topic, one email or social post), or heavy (podcast, narrated video, Refresh from Web / LIVE BOOKS research, YouTube package, multi-source Research & Import). You approve before anything expensive runs: heavy runs show a small pre-run dialog saying what's about to run, whose key pays (your own API key, your free allowance, or a genuinely free path like Mac voices or on-device AI), and an honest typical-cost range — ranges, never exact promises, since your provider does the billing. Cancel runs (and bills) nothing. Everyday light/medium tools run without interruption — no extra dialogs. The approval dialog has a per-feature 'Don't ask again' checkbox, and Professional mode in Settings suppresses all confirmations; Settings > 'Reset confirmation prompts' restores them. Every AI operation is also recorded in a private, LOCAL-ONLY usage ledger on the device (when it ran, which feature, which provider) — the user's own transparency record; nothing is sent anywhere.
...
- Slide Deck (Export menu, 2026-07-24): Turn an outline branch into a branded slide deck (.pptx) that opens in PowerPoint or Keynote. One slide per section (with its sub-points as bullets), plus a title slide and a closing slide, and an optional thought → idea → produce → publish arc slide. It reuses the app's brand look (dark/light theme, accent color, native IdeaM logo mark) so decks match the videos and the website. Numeric/percentage content (e.g. "80% use AI · 29% trust it · 45% lose time") is auto-turned into a native, still-editable bar chart. It's built straight from your outline — deterministic, no AI, no cost — and everything stays fully editable in PowerPoint or Keynote. Works on every platform (the file downloads to your Downloads folder).
- Refresh from Web (the web-refresh feature, internal code name LIVE BOOKS): Manually refresh a node and all its descendants against the latest information so documents don't go stale. Open it from the Import menu (down-arrow icon in the toolbar) > "Refresh from Web" (moved here 2026-06-06 — it brings external web content INTO existing nodes, so it fits the Import metaphor), the Command Palette, or Cmd/Ctrl+Shift+R (select the root node to refresh the entire outline). Choose Merge & augment (default — keep accurate content, fix outdated parts, fold in new info) or Overwrite/regenerate. By default every change is shown in a side-by-side preview with per-node accept/reject and nothing is applied until you approve; "Apply automatically without previewing" is an explicit opt-in that is off by default. Nodes you edited by hand are auto-skipped to protect your writing, with a per-node "Include anyway" override. Every refreshed node stores its sources and the model used — a compact "Sources" chip on the node shows the citations and attribution (e.g. "Refreshed with Gemini 3.5 Flash"), and this travels with the outline through save/export. Cloud AI uses live web search for real citations; local AI uses the model's built-in knowledge (no internet) and says so honestly rather than inventing sources. Refresh from Web is personal/local — it only changes your own copy after approval. It is built on a reusable transform engine that the Translate feature also uses.
- Proposed Changes — AI edits you approve (unified approval system, 2026-09-05): The AI never changes your outline without showing you first. Any AI action that would edit your work is marked right in the outline and waits for your decision — approve, adjust, or discard; nothing is saved until you approve, and Cmd+Z still undoes it afterward. The shared vocabulary: green tint + "Pending" badge = a new item to be added (choose Add or Discard); green tint + "New content" badge = new AI-written content inside an existing item; struck-through + amber "Will delete" badge = an item the AI was asked to remove (choose Delete or Keep); a Before/After side-by-side preview = a content rewrite of a single item (choose Approve or Discard). Covered actions: AI-command-bar deletes, Generate Suboutline from Topic, Generate Content for a node, and Create Content for Descendants (bulk) — the bulk review lists every filled item with a per-item keep/discard checkbox plus approve-all, and Discard restores everything exactly as it was. Refresh from Web and Translate use the same preview-and-approve pattern.
...
- Summarize outline (whole-outline gist, 2026-07-22): Distill the CURRENT outline down to its key points — a concise, well-organized GIST instead of every bullet. Built for detailed captures (imported video transcripts, comprehensive book outlines) that have grown so many bullets the main point is buried. Open from the Smart Tools menu > "Summarize outline" or the Command Palette > "Summarize outline". Always operates on the WHOLE current outline (root down), not the selected node. The dialog offers a depth choice — "Standard gist" (key points with light structure) or "Brief" (just the handful of most important points, maximum compression) — plus an output choice via the same "What should we do with the result?" radio the other transforms use: "Save as new outline" is the DEFAULT (non-destructive) and writes the gist into a brand-new outline named "[Original] — Summary" nested under the original, leaving the original untouched; "Replace this outline" overwrites in place, and because that is destructive the app shows a confirmation dialog before applying (auto-snapshot still fires, Cmd+Z still undoes). After running, a preview shows the condensed outline tree plus an "X nodes → Y" count. It REUSES the existing Transform Outline AI pipeline (a fixed "condense into a gist" instruction tuned by the depth toggle), so it inherits the same node-ID contract, the same Gemini/Ollama provider handling with automatic fallback, and respects the AI provider setting, the consent flow, and tier/generation limits. Optional "Use local AI (Ollama)" tickbox keeps it on-device. One summarize = ONE generation against the monthly cap, regardless of outline size. NOT a Pro-only feature.
- Professional Customization (Settings section, 2026-07-22): A Settings section (Settings > Professional Customization) that houses opt-in power features. Email tools are its first inhabitant and are STRICTLY OPT-IN and OFF BY DEFAULT — nothing email-related appears or runs anywhere in the app until the user turns them on. It has: a MASTER "Email tools" switch (off by default); a "Your email address" field (optional — targets the right Gmail account and personalizes the drafted email's sign-off, kept on-device); and granular per-feature sub-toggles shown only when the master is on ("Turn an outline into an email (Export Email)", on by default; "Import email into outlines", on by default; and — shown when Import is on — "File suspected junk aside", on by default). Flipping the master on the FIRST time shows an honest consent/warning (reachable again via an info button) that explains what it does, its advantages, the things to know (the AI reads the branch you select, governed by your AI Provider setting — on-device Gemma stays private, cloud AI sends that content to the provider; the Gmail/Mail hand-offs are plain text with a length cap so long emails are better via Copy or Download), and the privacy promise (IdeaM NEVER sends email for you and never touches your inbox — you always review and hit send yourself; turn it off anytime). Enabling requires an explicit Enable/Cancel choice.
- Capabilities / Project Management toggle (Settings section, 2026-07-27): Under Settings > Professional Customization there is a "Capabilities" section — opt-in switches that turn IdeaM into something more than plain notes + an outline. The FIRST capability is "Project Management", and it is OFF BY DEFAULT. When OFF (the default for everyone, including a student who just wants notes), all project-management surfaces are HIDDEN: the node right-click "Project" submenu (Set Status + Set Prerequisite), the colored status badges, the "Depends on" prerequisite chips and their "+ Add" button, and the "Blocked by" indicators. When the user switches Project Management ON, all of those appear. Turning it on the first time shows a light, dismissible honest-enablement note listing what it adds (status tracking, task dependencies, filter-by-status) with a "Turn on"/"Not now" choice; an info button re-opens that note. IMPORTANT: turning it OFF never deletes any node's status or prerequisites — it only hides the UI, and everything reappears exactly as it was when re-enabled. Basic free-form tags and Filter-by-tag are NOT project-management and stay available regardless. There is NO paywall — every capability is unlocked for everyone during the evaluation period. The design is extensible: future capabilities are added as more switches in this same Capabilities section.
- Export Email (branch-to-email wizard, 2026-07-22): Turn a selected BRANCH (a node + all its descendants — the same "chapter" scope Generate Video uses) into a ready-to-send email. GATED: only appears when the opt-in "Email tools" master switch is on (Settings > Professional Customization) — otherwise it is hidden everywhere. Open by right-clicking the branch or via the Export menu > "Export Email"; it's enabled once a node is selected. Works on desktop, web, and iOS. Flow: pick a tone (Friendly professional — the default, Formal, or Casual) and optionally add a one-line instruction (e.g. "keep it short", "address it to my team"), then click "Draft email". The AI writes a REAL email — a subject line plus a readable body with a greeting, connective prose and tidy bullets where they help, and a sign-off (personalized with your name from your saved email if set) — NOT a raw bullet dump. The result appears in an EDITABLE preview (subject field + body) so you can tweak before sending. Then FOUR no-login hand-offs: (a) "Open in Gmail" opens a Gmail compose window in your browser pre-filled with the subject and body via the Gmail compose URL (targets your account via authuser when your email is set; plain-text and length-limited; if your email is NOT set it shows a friendly "add your email in Settings > Professional Customization" prompt with a jump button instead of opening a broken Gmail — Copy/Download still work); (b) "Open in Mail" hands off to your DEFAULT mail app via a mailto: link (the universal any-provider / Apple Mail path, one implementation on every OS); (c) "Copy email" copies to the clipboard as BOTH rich text/html AND text/plain (with the subject at the top) so pasting into Gmail/Apple Mail keeps formatting; (d) "Download" saves a standard RFC-822 .eml file (Subject header, To: placeholder, MIME text/html + text/plain parts, X-Unsent so it opens as a draft) named from the subject, ready to open in Gmail/Apple Mail/Outlook. It REUSES the existing AI pipeline (Gemini with automatic Ollama fallback, BYOK keys, optional "Use local AI"), respects the AI provider setting/consent/limits, and counts as ONE generation. NOT Pro-only. Deliberately needs NO Google sign-in — saving a draft directly into a Gmail account (OAuth) is intentionally out of scope for now.
- Import Email (inbound email → outline, 2026-07-22): Bring an email or a whole thread INTO IdeaM as a clean, STRUCTURED outline — Summary, Key Points, Decisions, Action Items — NOT a wall of quoted text. GATED under the same opt-in framework: it appears in the toolbar's Import menu (down-arrow icon) as "Import Email" ONLY when the master "Email tools" switch AND the "Import email into outlines" sub-toggle are both on (Settings > Professional Customization); otherwise it is hidden everywhere. Flow: open Import > "Import Email", paste the email/thread text and/or drop a .eml file (the file is parsed to its readable text), choose the output — "Create new outline" (default, named from the subject, other outlines untouched), "Add to current outline", or "Save into Second Brain" (offered only when a Second Brain exists) — optionally tick "Use local AI (Ollama)", then Import. "File suspected junk aside" (sub-toggle, ON by default): when importing a thread the AI classifies each message as keep vs. suspected-junk and quarantines suspected junk into a clearly-labeled "Filtered — likely junk" sub-branch with a count — it NEVER deletes anything, so the user can glance and rescue; a single non-junk email produces no such branch. REUSES the existing import + AI pipeline (Gemini with automatic Ollama fallback, optional local AI), respects the AI provider setting / consent / generation limits, and counts as ONE generation (one import = one generation). NOT Pro-only.
- Your Voice (personal writing-style emulation, 2026-07-22): Teach IdeaM your OWN writing style, then generate outputs that sound like you. STRICTLY your own voice — there is NO capability to imitate someone else or a public figure anywhere. Opt-in and OFF BY DEFAULT, its own entry in Settings > Professional Customization (independent of the Email tools master, because Your Voice applies across many output wizards, not just email). Turn on the master "Your Voice" switch, then build a VOICE PROFILE from SAMPLES of your own writing: paste a few things you've written (posts, emails, notes — a few hundred words is plenty) and/or click "Learn from my Second Brain" to pull a sample of your own saved Second Brain content into the box. Click "Learn my voice" and the AI distills a concise, reusable profile describing your tone, formality, sentence rhythm, vocabulary, emoji/hashtag habits, and quirks. The distilled profile is shown in an EDITABLE box so you can see and tweak what the app learned; regenerate anytime as you add more samples (each generation = ONE generation against the monthly cap). Once a profile exists, an "In my voice" option appears in the output wizards (Export Email and Summarize today; structured so upcoming social-export formats reuse the same profile) — tick it and the output is written in your style by injecting the voice profile into the AI prompt. When Your Voice is off or no profile exists, the option is hidden and generation behaves normally. You always review and edit both the profile and every generated output. REUSES the existing AI pipeline (Gemini with automatic Ollama fallback, BYOK keys, optional "Use local AI"), respects the AI provider setting/consent/limits. NOT Pro-only. (Groundwork for a future opt-in "Teach Mode" that could refine the profile from your edits — not built yet.)
- Social export & Share to X (first social-media output format, 2026-07-22): Turn a selected BRANCH (a node + all its descendants — the same "chapter" scope Generate Video / Export Email use) into ready-to-post social content. Built on a lightweight, EXTENSIBLE "social format template" pattern so more platforms (Instagram, LinkedIn, Facebook, Threads, Bluesky, YouTube, short-form video) slot in as new templates, not new plumbing — X is the first instance. GATED by its own opt-in master "Social export" switch in Settings > Professional Customization — OFF BY DEFAULT and INDEPENDENT of Email tools and Your Voice; when off the "Share to Social" action is hidden everywhere. First enable shows a SHORT honest note (lighter than the email consent): what it does, that the AI reads the branch per your AI Provider setting (on-device private / cloud sends to provider), and that IdeaM NEVER posts anything automatically and never connects to your accounts — you always review and post yourself (Enable/Cancel; reachable again via an info button). A per-platform sub-toggle ("Share to X") sits under the master. Open via right-click the branch / Export menu / Command Palette > "Share to Social"; enabled once a node is selected. Flow: pick the platform (X for now) and a FORMAT — "Thread" (a hook post first, then follow-on posts, each within X's ~280-char limit, with natural hashtags) or "Single post" (the whole branch condensed into one ≤280-char post); if Your Voice is on and a profile exists, tick "In my voice" to reuse that SAME saved voice profile so posts sound like you; optionally add a one-line instruction; optional "Use local AI". Click "Write thread"/"Write post"; every post is guaranteed within the char limit by a safety splitter. EDITABLE preview: one box per post with a live character count. FOUR no-login hand-offs (no X API, no OAuth, no auto-posting): (a) "Copy thread" copies all posts numbered (1/5, 2/5…) ready to paste; (b) "Copy" per post copies one; (c) "Open in X" opens X's web compose intent (https://twitter.com/intent/tweet?text=<url-encoded first post>) prefilled with ONLY the first post — the UI makes clear the rest are copied for pasting as replies (that's all X's intent allows without an account connection); (d) "Download" saves the posts as a .txt. On Electron, external URLs open via the external-link bridge; on web via window.open. REUSES the existing AI pipeline (Gemini with automatic Ollama fallback, BYOK keys), respects the AI provider setting/consent/limits, counts as ONE generation. NOT Pro-only (no paywall — pricing deferred). Deliberately NO direct posting via the X API and no account connection — copy / open-intent / download only.
- Share to Instagram (second social format template, 2026-07-22): Under the SAME opt-in "Social export" master switch (Settings > Professional Customization) with its own "Share to Instagram" sub-toggle; added as a new social template (no new plumbing). Selected in the Share to Social dialog's platform picker (X or Instagram). Two FORMATS: (1) CAPTION — the AI writes an Instagram caption plus natural hashtags (optional "In my voice"); hand-offs are Copy caption (caption + hashtags) and Download .txt. (2) CAROUSEL (the showpiece) — the AI writes a short punchy line per slide (slide 1 = a hook cover), and the app RENDERS each as a REAL 1080×1080 BRANDED SQUARE PNG image reusing the Generate Video branding (theme dark/light, accent colour, brand name, logo) — drawn on an HTML5 canvas so it works on web, Electron, and iOS (the 16:9 Electron video pipeline is not reused because it is desktop-only and non-square). The user picks theme/accent/brand in the dialog. It also generates the matching caption + hashtags (optional "In my voice" on the caption). Hand-offs: "Download carousel" saves all slides in a .zip (via jszip), and "Copy caption". HONEST hand-off: Instagram has NO usable web compose intent, so there is deliberately NO fake "Open in Instagram" button — instead a note says "Instagram posts from your phone — download these and post from the Instagram app." Reuses the AI pipeline (Gemini with Ollama fallback, BYOK, optional local AI), counts as ONE generation, NOT Pro-only, no paywall, never posts for you.
- Share to LinkedIn, Facebook, Threads & Bluesky (four more social templates, 2026-07-22): Added on the SAME "social format template" pattern (no new plumbing) under the SAME opt-in "Social export" master switch, each with its own per-platform sub-toggle in Settings > Professional Customization. All appear in the Share to Social dialog's platform picker, respect each network's character norms in the AI prompt, and support the optional "In my voice" (reusing the SAME Your Voice profile). (1) LinkedIn — a professional, story-arc post: strong standalone opening line, 1-3 short paragraphs with white space, a few tasteful hashtags (~3-5), within 3000 chars. (2) Facebook — a friendly, conversational post, more casual than LinkedIn, hashtags minimal (0-2), within 2000 chars. (3) Threads — casual, punchy; single post or a short numbered SEQUENCE, each post within 500 chars. (4) Bluesky — one casual post within ~300 chars. HONEST per-platform hand-offs: Threads and Bluesky DO support a public compose intent, so they get a one-click "Open in Threads" (prefills the first post; rest copied to paste as replies) / "Open in Bluesky" button, PLUS Copy and Download .txt. LinkedIn and Facebook have NO reliable pre-filled TEXT compose (their sharers only take a URL), so there is deliberately NO fake "Open in…" button — the dialog shows an honest copy-and-paste-it-yourself note; hand-offs are Copy and Download .txt only. On Electron external URLs open via the external-link bridge; on web via window.open. Reuses the AI pipeline, counts as ONE generation, NOT Pro-only, no paywall, no direct-posting API, never connects to your accounts.
- Share to YouTube (final social template, 2026-07-23): The last member of the Share to Social lineup, added on the SAME "social format template" pattern (outputKind 'youtube', no new plumbing) under the SAME opt-in "Social export" master switch, with its own "Share to YouTube" per-platform sub-toggle in Settings > Professional Customization. Natural fit because the app already GENERATES video and already IMPORTS from YouTube. Its value is the AI-written PUBLISH PACKAGE that makes an outline video ready to post. Selected in the Share to Social dialog's platform picker. Two FORMATS (variants): (1) "Publish package" (standard) — the AI writes a few SEO-friendly TITLE options, a full DESCRIPTION with a chapter-timestamp section DERIVED FROM THE BRANCH'S STRUCTURE (top-level sub-points become chapters, timestamps spaced evenly from 0:00), a TAGS list, and a THUMBNAIL text/idea. (2) "Shorts" — a punchy vertical idea: a short hook title + a tight script for a <60s vertical clip (metadata/script only; no vertical-video rendering yet — ties to future short-form video). Optional "In my voice" reuses the SAME Your Voice profile for the title/description tone. PAIRS WITH Generate Video: this writes the words; the actual MP4 comes from the existing Generate Video feature — a "Generate the video" button jumps straight there (reuses it; video is NOT reimplemented here). HONEST hand-off — NO fake auto-upload: a YouTube upload needs a signed-in account and can't be pre-filled without the OAuth Data API (out of scope), so there is deliberately NO "upload to YouTube" button. Hand-offs are Copy title / Copy description / Copy tags / Copy all, Download the package as .txt, and an optional "Upload page" link that just opens https://youtube.com/upload (the real upload page; it CANNOT be pre-filled — the copied fields are pasted there by the user). An amber note reads: "Generate your video, then upload it to YouTube and paste this title, description, and tags." On Electron the upload link opens via the external-link bridge; on web via window.open. Reuses the AI pipeline (Gemini with Ollama fallback, BYOK, optional local AI), counts as ONE generation, NOT Pro-only, no paywall, never posts for you. (Distinct from the separate heavier "YouTube package" Premium dialog, which produces a full production kit — voiceover script, b-roll prompts, shot list — and is not part of the Share to Social lineup.)
...
- Import/Export value tooltips (2026-07-10): The Import (down-arrow) and Export (up-arrow) toolbar icons carry outcome-based tooltips — Import: "bring in YouTube, PDFs, web pages, notes, audio, or another outline"; Export: "video, podcast, website, or 20+ formats".
